...
@section('content')
    <div class="col-lg-6 mb-5">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Edit SIM List</h1>
        </div>
        <form method="post" action="/simList-backend/{{ $simList->id }}">
            @method('PUT')
            @csrf
            ...
            <div class="form-group mb-5">
                ...
                <input type="text" class="form-control form-control-user @error('number') is-invalid @enderror"
                    placeholder="1234-1234-123456" value="{{ $simList->number, old('number') }}" name="number">
                @error('number')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
